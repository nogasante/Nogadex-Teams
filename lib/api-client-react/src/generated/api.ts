 ...
 */
export const getGetRoomParticipantsUrl = (roomId: string) => {
...
export const getRoomParticipants = async (
  roomId: string,
  options?: RequestInit,
): Promise<GetRoomParticipants200> => {
  return customFetch<GetRoomParticipants200>(
    getGetRoomParticipantsUrl(roomId),
    {
      ...options,
      method: "GET",
    },
...
export const getGetRoomParticipantsQueryKey = (roomId: string) => {
  return [`/api/rooms/${roomId}/participants`] as const;
};
...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? getGetRoomParticipantsQueryKey(roomId);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getRoomParticipants>>
  > = ({ signal }) =>
    getRoomParticipants(roomId, { signal, ...requestOptions });
...
>(
  ...
  const queryOptions = getGetRoomParticipantsQueryOptions(roomId, options);

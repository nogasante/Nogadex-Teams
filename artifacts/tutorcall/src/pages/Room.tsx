...
interface FloatingReaction { id: string; emoji: string; senderName: string; x: number; }
...
export default function Room() {
  ...
  // Read from sessionStorage — these are set by Home.tsx before navigation
  const userName = sessionStorage.getItem("tutorcall-username");
  // isHost is determined at join/create time, NOT from the room API
  // This prevents the WebRTC effect from re-running when room data loads
  const isHost = sessionStorage.getItem("tutorcall-is-host") === "true";
  ...
  // Guard: must have a username
  useEffect(() => {
    if (!userName) setLocation("/");
  }, [userName, setLocation]);

  // ── WebRTC — passes stable isHost from sessionStorage ──
  const {
    localStream,
    peers,
    chatMessages,
  ...
  useEffect(() => {
    ...
    const floating: FloatingReaction = { id: latest.id, emoji: latest.emoji, senderName: latest.senderName, x: 15 + Math.random() * 70 };
    setFloatingReactions(prev => [...prev, floating]);
    const t = setTimeout(() => setFloatingReactions(prev => prev.filter(r => r.id !== floating.id)), 3200);
  ...
  const handleLeave = () => {
    ...
    sessionStorage.removeItem("tutorcall-is-host");
    setLocation("/");
  };
  ...
  const handleToggleHand = () => { if (isHandRaised) lowerHand(); else raiseHand(); };

  // ── Room not found ──
  if (isRoomError) {
    return (
      <div className="room-bg min-h-screen flex items-center justify-center p-6">
        <div className="glass-dark rounded-3xl p-8 flex flex-col items-center gap-4 max-w-sm w-full">
          <AlertCircle className="w-10 h-10" style={{ color: "hsl(0 65% 55%)" }} />
          <h2 className="text-lg font-semibold" style={{ color: "hsl(38 20% 88%)" }}>Room not found</h2>
          <p className="text-sm text-center" style={{ color: "hsl(220 5% 55%)" }}>
            The room <span className="font-mono font-semibold" style={{ color: "hsl(38 20% 72%)" }}>{roomId}</span> doesn't exist or has ended.
          </p>
          <button
            onClick={() => setLocation("/")}
            className="glass-burgundy rounded-xl px-6 py-2.5 text-sm font-semibold text-white/90"
          >
            Back to Home
          </button>
        </div>
  ...
  const spotlightedPeer = spotlightedPeerId && spotlightedPeerId !== socketId ? peers[spotlightedPeerId] : null;
    ...
    <div className="room-bg w-full flex flex-col overflow-hidden select-none" style={{ fontFamily: "Inter, sans-serif", height: "100dvh" }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-50">
        {floatingReactions.map(r => (
          <div key={r.id} className="absolute bottom-28 flex flex-col items-center gap-1"
            style={{ left: `${r.x}%`, animation: "floatUp 3.2s ease-out forwards" }}>
        ...
        <div className="flex items-center gap-3">
          ...
          {isHost && (
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full"
              style={{ background: "rgba(123,29,50,0.55)", color: "hsl(38 35% 88%)" }}>Host</span>
          )}
          <span className="text-xs" style={{ color: "hsl(38 10% 58%)" }}>{userName}</span>
          {audioOnly && (
            <span className="text-[10px] px-2 py-0.5 rounded-full"
              style={{ background: "rgba(255,248,240,0.08)", color: "hsl(38 10% 52%)" }}>Audio</span>
          )}
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-3 flex flex-col gap-3 overflow-hidden">
          {isLoadingRoom && !localStream ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="glass-dark rounded-2xl px-8 py-6 text-white/60 text-sm">Connecting…</div>
            </div>
          ) : hasSpotlight ? (
            <div className="flex-1 flex flex-col gap-3 overflow-hidden">
              ...
              <div className="flex-1 min-h-0 rounded-2xl overflow-hidden relative ring-burgundy"
                ...
                <VideoPlayer stream={mainStageStream} name={mainStageName || ""} isHost={mainStageIsHost}
                  isMuted={mainStageIsMuted} isVideoOff={mainStageIsVideoOff}
                  isLocal={spotlightedPeerId === socketId} className="h-full rounded-none" />
                {isHost && (
                  <button onClick={() => spotlightParticipant(null)}
              ...
              <div className="h-32 shrink-0 flex gap-2.5 overflow-x-auto">
                {spotlightedPeerId !== socketId && (
                  <PeerTile stream={localStream} name={userName} isHost={isHost} isMuted={isMuted}
                    isVideoOff={isVideoOff} isLocal isHandRaised={isHandRaised}
                    onPin={isHost ? () => spotlightParticipant(socketId) : undefined} />
                )}
                {peersList.filter(p => p.socketId !== spotlightedPeerId).map(peer => (
                  <PeerTile key={peer.socketId} stream={peer.stream} name={peer.name} isHost={peer.isHost}
                    isMuted={peer.isMuted} isVideoOff={peer.isVideoOff} isHandRaised={peer.handRaised}
            ...
            <div className={cn("flex-1 grid gap-3 overflow-hidden auto-rows-fr", gridCols)}>
              <PeerTile stream={localStream} name={userName} isHost={isHost} isMuted={isMuted}
                isVideoOff={isVideoOff} isLocal isHandRaised={isHandRaised} large
                onPin={isHost ? () => spotlightParticipant(socketId) : undefined} />
              {peersList.map(peer => (
                <PeerTile key={peer.socketId} stream={peer.stream} name={peer.name} isHost={peer.isHost}
                  isMuted={peer.isMuted} isVideoOff={peer.isVideoOff} isHandRaised={peer.handRaised} large
                  onPin={isHost ? () => spotlightParticipant(peer.socketId) : undefined}
                  onMute={isHost ? () => muteParticipant(peer.socketId) : undefined}
                  onLowerHand={isHost && peer.handRaised ? () => hostLowerHand(peer.socketId) : undefined} />
              ))}
            ...
            {sidePanel === "chat" ? (
              <>
                <div className="h-12 px-4 flex items-center gap-2 border-b" style={{ borderColor: "rgba(255,248,240,0.08)" }}>
                  ...
                  <span className="text-xs px-1.5 py-0.5 rounded-full"
                    style={{ background: "rgba(255,248,240,0.10)", color: "hsl(38 10% 58%)" }}>
                    {chatMessages.length}
                  </span>
                </div>
                <div className="flex-1 overflow-y-auto p-3.5 space-y-3" ref={chatScrollRef}>
                  {chatMessages.length === 0 ? (
                    ...
                    <div key={msg.id} className="space-y-1">
                      ...
                      <div className="rounded-2xl rounded-tl-sm px-3 py-2 text-xs leading-relaxed"
                ...
                <form onSubmit={handleSendChat} className="p-3 border-t flex gap-2"
                  ...
                  <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                    placeholder="Message…" className="flex-1 rounded-xl px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-[hsl(345_60%_45%)]"
                    style={{ background: "rgba(255,248,240,0.08)", border: "1px solid rgba(255,248,240,0.12)", color: "hsl(38 15% 88%)" }} />
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-1">
                  <ParticipantRow name={`${userName} (you)`} isHost={isHost} isMuted={isMuted}
                    isVideoOff={isVideoOff} isHandRaised={isHandRaised} audioOnly={audioOnly} />
                  {peersList.map(peer => (
                    <ParticipantRow key={peer.socketId} name={peer.name} isHost={peer.isHost}
                      isMuted={peer.isMuted} isVideoOff={peer.isVideoOff} isHandRaised={peer.handRaised}
                      onMute={isHost ? () => muteParticipant(peer.socketId) : undefined}
                      onPin={isHost ? () => spotlightParticipant(peer.socketId) : undefined}
                      onLowerHand={isHost && peer.handRaised ? () => hostLowerHand(peer.socketId) : undefined} />
      ...
      <div className="glass-dark sm:h-20 h-24 px-4 sm:px-6 flex items-center justify-between shrink-0 z-20 border-t pb-safe"
        ...
        <div className="flex-1 flex items-center gap-2">
          <div className="relative">
            ...
            {showReactionPicker && (
              <div className="absolute bottom-14 left-0 glass-dark rounded-2xl p-2 flex flex-wrap gap-1.5"
                style={{ width: 216, border: "1px solid rgba(255,248,240,0.12)" }}>
                {REACTION_EMOJIS.map(emoji => (
                  <button key={emoji} onClick={() => { sendReaction(emoji); setShowReactionPicker(false); }}
        ...
        <div className="flex-1 flex items-center justify-end gap-2">
          <PanelToggle active={sidePanel === "participants"} onClick={() => togglePanel("participants")} count={peersList.length + 1}>
            <Users className="w-4 h-4" />
          </PanelToggle>
          <PanelToggle active={sidePanel === "chat"} onClick={() => togglePanel("chat")} count={chatMessages.length}>
...
function PeerTile({ stream, name, isHost, isMuted, isVideoOff, isLocal, isHandRaised, onPin, onMute, onLowerHand, large }: PeerTileProps) {
  return (
    <div className={cn("rounded-2xl overflow-hidden relative group shrink-0", large ? "h-full" : "w-52 h-full")}
      style={{ border: "1px solid rgba(255,248,240,0.12)" }}>
      <VideoPlayer stream={stream} name={name} isHost={isHost} isMuted={isMuted} isVideoOff={isVideoOff}
        isLocal={isLocal} className="h-full rounded-none" />
      {isHandRaised && (
        <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-xl text-xs font-semibold"
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="glass-btn rounded-xl p-1.5"><MoreVertical className="h-3.5 w-3.5 text-white/70" /></button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-44 glass-dark border-white/10 text-white/80 text-sm">
              {onMute && <DropdownMenuItem onClick={onMute} className="cursor-pointer focus:bg-white/10"><MicOff className="mr-2 h-3.5 w-3.5" /> Mute mic</DropdownMenuItem>}
              {onPin && <DropdownMenuItem onClick={onPin} className="cursor-pointer focus:bg-white/10"><Pin className="mr-2 h-3.5 w-3.5" /> Pin to stage</DropdownMenuItem>}
...
interface ParticipantRowProps {
  name: string; isHost?: boolean; isMuted?: boolean; isVideoOff?: boolean;
  isHandRaised?: boolean; audioOnly?: boolean;
  onMute?: () => void; onPin?: () => void; onLowerHand?: () => void;
}
function ParticipantRow({ name, isHost, isMuted, isVideoOff, isHandRaised, audioOnly, onMute, onPin, onLowerHand }: ParticipantRowProps) {
  ...
  return (
    <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl group" style={{ background: "rgba(255,248,240,0.04)" }}>
      <div className="w-7 h-7 rounded-xl flex items-center justify-center text-[10px] font-bold shrink-0"
        style={{ background: "rgba(123,29,50,0.45)", color: "hsl(38 35% 88%)" }}>{initials}</div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {isMuted ? <MicOff className="w-3.5 h-3.5" style={{ color: "hsl(0 60% 55%)" }} /> : <Mic className="w-3.5 h-3.5" style={{ color: "hsl(142 40% 50%)" }} />}
        {isVideoOff ? <VideoOff className="w-3.5 h-3.5" style={{ color: "hsl(0 60% 55%)" }} /> : <VideoIcon className="w-3.5 h-3.5" style={{ color: "hsl(142 40% 50%)" }} />}
        {hasActions && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="ml-1 opacity-0 group-hover:opacity-100 transition-opacity"><MoreVertical className="w-3.5 h-3.5 text-white/40" /></button>
            </DropdownMenuTrigger>
...
interface CtrlBtnProps {
  ...
  active?: boolean; destructive?: boolean; label?: string; style?: React.CSSProperties;
}
...
interface PanelToggleProps { children: React.ReactNode; active: boolean; onClick: () => void; count?: number; }
function PanelToggle({ children, active, onClick, count }: PanelToggleProps) {
  return (
    <button onClick={onClick}
      className={cn("relative flex items-center gap-1.5 rounded-xl px-3.5 h-9 text-xs font-medium transition-all",
